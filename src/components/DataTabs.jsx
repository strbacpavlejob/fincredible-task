...
export const DataTabs = ({ bankData }) => {
        ...
        switch (selectedTabIndex) {
            ...
            case 2: {
                const verificationObject = {
                    "IBAN Check": verificationHelper(personalData.IBAN.includes(personalData.Country)),
                    "DEBT_RISK": verificationHelper(Accounts.CurrentBalance < 0, true),
                    "GAMBLING": verificationHelper(Accounts.Transaction.any(item => item.Description.includes("gambling"))),
                    "CHILD_SUPPORT": verificationHelper(Accounts.Transaction.any(item => item.Description.includes("support"))),
                }
                ...
                <table>
                    {
                        Object.entries(verificationObject).map(({ key, value }) => {
                            ...
                            (
                                ...
                                <tr style={{ backgroundColor: getColor(value) }}>
                                    <td>{key}</td>
                                    <td>{value}</td>
                                </tr>
    ...
    (<>
        {tabs.map((item, index) => {
            return <div onClick={() => setSelectedTabIndex(index)}>{item}</div>
        })}
        <TabBody />
    </>);
}